  ...
  <button
    ...
    className={`px-4 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${
      active
        ...
        : 'bg-white text-[#526477] hover:text-[#102033] hover:bg-slate-50 border border-slate-200/80 shadow-sm'
    }`}
  >
    {label}
    {badge != null && badge > 0 && (
    ...
    <PageContainer
      ...
      actions={
        <button
          ...
          className="p-2 rounded-xl bg-white hover:bg-slate-50 border border-slate-200/80 text-[#526477] hover:text-[#102033] shadow-sm transition-colors"
          title="Refresh"
        >
        ...
        <div className="space-y-5">
          ...
          <div className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
            <h3 className="text-base font-bold text-[#102033] mb-4 flex items-center gap-2">
              <GraduationCap className="w-4 h-4 text-[#006AC7]" />
              My Profile
            </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs uppercase text-[#526477] font-bold tracking-wider">Active Homework</span>
              </div>
              <p className="text-3xl font-black text-[#102033]">{hwLoaded ? homework.length : '—'}</p>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs uppercase text-[#526477] font-bold tracking-wider">Due Soon (48h)</span>
              </div>
              <p className="text-3xl font-black text-[#102033]">{hwLoaded ? dueSoon.length : '—'}</p>
            </div>
            <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs uppercase text-[#526477] font-bold tracking-wider">Overdue</span>
              </div>
              <p className="text-3xl font-black text-[#102033]">{hwLoaded ? overdue.length : '—'}</p>
            </div>
          </div>

          {/* Overdue alert */}
          {hwLoaded && overdue.length > 0 && (
            <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 flex items-start gap-3 text-sm font-medium">
          ...
          <div className="p-5 rounded-2xl bg-[#F0F8FF]/80 border border-blue-100 text-[#006AC7] text-sm font-medium flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 flex-shrink-0 mt-0.5 text-[#006AC7]" />
        ...
        <div className="space-y-4">
          {hwLoading ? (
            ...
            <div className="p-10 rounded-2xl bg-white border border-slate-200/80 shadow-sm text-center">
              ...
              <p className="text-[#102033] font-bold">All caught up! No active homework.</p>
              <p className="text-xs text-[#526477] mt-1">Check back later for new class assignments.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {homework.map((hw) => {
                ...
                return (
                  <div key={hw._id} className={`p-5 rounded-2xl border transition-all shadow-sm ${
                    isOverdue  ? 'bg-rose-50/40 border-rose-200' :
                    isDueSoon  ? 'bg-amber-50/40 border-amber-200' :
                                  'bg-white border-slate-200/80'
                  }`}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-[#102033] text-sm mb-1 truncate">{hw.title}</p>
                        {hw.description && (
                          <p className="text-xs text-[#526477] mb-2 line-clamp-2">{hw.description}</p>
                        )}
                        <div className="flex flex-wrap gap-2 text-xs">
                          <span className="px-2.5 py-1 rounded-full bg-blue-50 text-[#006AC7] border border-blue-100 font-bold">
                            {hw.subjectId?.name || 'Subject'}
                          </span>
                          <span className="px-2.5 py-1 rounded-full bg-slate-100 text-[#526477] font-medium">
                            By: {hw.teacherId?.fullName || 'Teacher'}
                          </span>
                          {hw.attachments?.length > 0 && (
                            <span className="px-2.5 py-1 rounded-full bg-purple-50 text-purple-700 border border-purple-100 font-medium flex items-center gap-1">
                      ...
                      <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                        <span className={`text-xs px-2.5 py-1 rounded-full font-bold border ${
                          ...
                          isDueSoon  ? 'bg-amber-50 text-amber-700 border-amber-200' :
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-100 space-y-1.5">
                        {hw.attachments.map((att, i) => (
                          <a
                            ...
                            className="flex items-center gap-2 text-xs text-[#006AC7] hover:underline font-medium transition-colors"
                          >
                            <FileText className="w-3 h-3" />
                            <span className="truncate">{att.fileName || 'Attachment'}</span>
                            <span className="text-[#8094A8]">({att.fileType})</span>
                          </a>
                        ))}
                      </div>
                    )}
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              disabled={attLoading}
              className="px-3 py-1.5 rounded-xl text-xs font-bold bg-white text-[#526477] hover:text-[#102033] hover:bg-slate-50 border border-slate-200/80 shadow-sm flex items-center gap-1.5 transition-colors"
            >
          ...
          {attLoading && !attendance ? (
            ...
            <div className="p-8 rounded-2xl bg-white border border-slate-200/80 shadow-sm text-center text-[#526477]">
              ...
              <p className="font-medium">Attendance records not yet initialized for your profile.</p>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {/* 1. Current Month */}
                <div className="p-5 rounded-2xl bg-white border border-blue-200/80 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-slate-100 text-center text-xs">
                    <div className="p-1.5 rounded-lg bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-700 font-bold">{attendance.currentMonth?.present || 0}</span>
                      <p className="text-[10px] text-[#526477]">Present</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-rose-50 border border-rose-100">
                      <span className="text-rose-700 font-bold">{attendance.currentMonth?.absent || 0}</span>
                      <p className="text-[10px] text-[#526477]">Absent</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-amber-50 border border-amber-100">
                      <span className="text-amber-700 font-bold">{attendance.currentMonth?.leave || 0}</span>
                ...
                {/* 2. Last Month */}
                <div className="p-5 rounded-2xl bg-white border border-indigo-200/80 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-slate-100 text-center text-xs">
                    <div className="p-1.5 rounded-lg bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-700 font-bold">{attendance.lastMonth?.present || 0}</span>
                      <p className="text-[10px] text-[#526477]">Present</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-rose-50 border border-rose-100">
                      <span className="text-rose-700 font-bold">{attendance.lastMonth?.absent || 0}</span>
                      <p className="text-[10px] text-[#526477]">Absent</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-amber-50 border border-amber-100">
                      <span className="text-amber-700 font-bold">{attendance.lastMonth?.leave || 0}</span>
                      <p className="text-[10px] text-[#526477]">Leave</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-2xl bg-white border border-amber-200/80 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-bold text-amber-600 uppercase tracking-wider">Academic Session</span>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 pt-2 border-t border-slate-100 text-center text-xs">
                    <div className="p-1.5 rounded-lg bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-700 font-bold">{attendance.academicYear?.present || 0}</span>
                      <p className="text-[10px] text-[#526477]">Total P</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-rose-50 border border-rose-100">
                      <span className="text-rose-700 font-bold">{attendance.academicYear?.absent || 0}</span>
                      <p className="text-[10px] text-[#526477]">Total A</p>
                    </div>
                    <div className="p-1.5 rounded-lg bg-amber-50 border border-amber-100">
                      <span className="text-amber-700 font-bold">{attendance.academicYear?.leave || 0}</span>
                ...
                <div className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {attendance.monthlyHistory.map((m, idx) => (
                      <div key={idx} className="flex items-center gap-4 p-3 rounded-xl bg-slate-50 border border-slate-200/60">
                        <span className="w-14 text-xs font-bold text-[#526477]">{m.monthLabel}</span>
                        <div className="flex-1">
                          <div className="h-2.5 rounded-full bg-slate-200 overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${
                                m.percentage >= 85 ? 'bg-[#4B7F3A]' :
                                m.percentage >= 75 ? 'bg-[#006AC7]' :
                                'bg-rose-500'
                              }`}
                        ...
                        <div className="text-right w-24">
                          <span className="text-xs font-bold text-[#102033]">{m.percentage}%</span>
                          <span className="text-[10px] text-[#8094A8] block font-medium">({m.presentDays}/{m.totalWorkingDays} d)</span>
                        </div>
                      </div>
                    ))}
                  </div>
...
const InfoRow = ({ label, value, highlight }) => (
  <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-200/60">
    <span className="text-[#526477] font-medium">{label}</span>
    <span className={`font-bold ${highlight ? 'text-[#006AC7] font-mono' : 'text-[#102033]'}`}>{value || '—'}</span>
  </div>
);
