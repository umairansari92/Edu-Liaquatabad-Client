...
export const UserAuthorityModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Canonical authority grant endpoint — enforces full transition matrix,
      // self-grant prohibition, ROOT_ADMIN immutability, and tokenVersion invalidation.
      // Payload is strictly { authority, scope, reason } — designation is NOT changed here.
      const response = await apiClient.post(`/admin/users/${targetUser._id}/authority`, {
        authority: selectedAuthority,
        scope: selectedScope,
        reason: reasonText.trim(),
      });

      toast.success('System authority granted successfully. All existing sessions for this account have been revoked.');
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
          ...
          {!canActorManageTarget ? (
            ...
            <form id="authority-form" onSubmit={handleSubmit} className="space-y-5">
              {/* Civil Designation — read-only context display, NOT sent to server */}
              <div>
                <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1.5">
                  Civil Service Designation (Current — Not Modified by This Operation)
                </label>
                <div className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950/60 border border-slate-800/60 text-amber-300 text-sm font-medium flex items-center gap-2">
                  <Lock className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                  <span>{designationText || 'Not set'}</span>
                </div>
                <p className="text-[11px] text-slate-500 mt-1">
                  Designation is a descriptive civil title and is not changed by authority grant. To update designation, use the role-designation endpoint separately.
                </p>
              </div>
                ...
                <div className="p-4 rounded-xl bg-amber-950/30 border border-amber-800/40 text-amber-300 text-xs space-y-2 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px] pt-1 border-t border-amber-800/30">
                    ...
                    <div>
                      ...
                      <span className="text-slate-500 italic text-[10px]">Unchanged — not sent</span>
                    </div>
                  </div>
